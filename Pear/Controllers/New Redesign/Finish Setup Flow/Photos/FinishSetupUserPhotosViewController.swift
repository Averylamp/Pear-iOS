//
//  FinishSetupUserPhotosViewController.swift
//  Pear
//
...
class FinishSetupUserPhotosViewController: UIViewController {
  ...
  var imageBank: [LoadedImageContainer] = []
  ...
  class func instantiate(displayedImages: [LoadedImageContainer],
                         imageBank: [LoadedImageContainer]) -> FinishSetupUserPhotosViewController? {
    let storyboard = UIStoryboard(name: String(describing: FinishSetupUserPhotosViewController.self), bundle: nil)
    guard let photoInputVC = storyboard.instantiateInitialViewController() as? FinishSetupUserPhotosViewController else { return nil }
  ...
  @IBAction func nextButtonClicked(_ sender: Any) {
    ...
    let displayedImages = self.images.compactMap({ $0.imageContainer })
                                        ...
                                        DispatchQueue.main.async {
                                          ...
                                          case .success(let success):
                                            if success {
                                              ...
                                              DataStore.shared.refreshPearUser { (_) in
                                                DispatchQueue.main.async {
                                                  guard let mainVC = LoadingScreenViewController.getMainScreenVC()
                                                    else {
...
// MARK: - Life Cycle
extension FinishSetupUserPhotosViewController {
  override func viewDidLoad() {
    super.viewDidLoad()
    self.setupCollectionView()
    self.longPressGestureRecognizer = UILongPressGestureRecognizer(target: self, action: #selector(FinishSetupUserPhotosViewController.handleLongGesture(gesture:)))
...
extension FinishSetupUserPhotosViewController: UICollectionViewDelegate {
...
extension FinishSetupUserPhotosViewController: UICollectionViewDataSource, ImageUploadCollectionViewDelegate {
...
extension FinishSetupUserPhotosViewController: UICollectionViewDelegateFlowLayout {
