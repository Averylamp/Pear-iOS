...
class ApproveDetachedProfilePhotosViewController: UIViewController {
  ...
  class func instantiate(detachedProfile: PearDetachedProfile,
                         displayedImages: [LoadedImageContainer],
                         imageBank: [LoadedImageContainer]) -> ApproveDetachedProfilePhotosViewController? {
...
//      alertController.addAction(viewImageAction)
  ...
  func pickImage() {
    if UIImagePickerController .isSourceTypeAvailable(UIImagePickerController.SourceType.camera) {
      if self.imageReplacementIndexPath != nil {
        self.imagePickerController.settings.selection.max = 6 - self.images.count + 1
      } else {
        self.imagePickerController.settings.selection.max = 6 - self.images.count
      }
      
      self.presentImagePicker(self.imagePickerController, animated: true, select: { (_) in
  ...
  func openCamera() {
    if UIImagePickerController .isSourceTypeAvailable(UIImagePickerController.SourceType.camera) {
      self.imagePickerController.settings.selection.max = 6 - self.images.count
      self.presentImagePicker(self.imagePickerController, animated: true, select: nil, deselect: nil, cancel: nil, finish: { (assets) in
        self.newPicturesSelected(assets: assets)
      }, completion: nil)
    } else {
